...
Lines 10-32 listed source files twice (`.cpp` files duplicated, `.h` files mixed in). Fixed to a single clean list of `.cpp` files only:
...
1. `cmake --preset linux-debug` (reconfigure after CMakeLists fix)
2. `cmake --build --preset linux-debug` — must compile with zero errors
3. Run: the app still prints the stub message (main.cpp unchanged), but VulkanContext.cpp compiles cleanly
4. Full integration test comes when we wire main.cpp + SwapChain (next step)

---

## Next Steps (after VulkanContext)
...
| 6 | Shaders | triangle.vert / triangle.frag (already written) |
